...
class ViewController: UIViewController ,AVAudioPlayerDelegate {
    ...
    func initializePlayer(){
        ...
        do {
            ...
            print("플레이어 초기화 실패")
            print("오류코드 \(error.code), 메세지 : \(error.localizedDescription)")
        }
    ...
    func audioPlayerDecodeErrorDidOccur(_ player: AVAudioPlayer, error: Error?) {
        ...
        let okAction: UIAlertAction = UIAlertAction(title: "확인", style: UIAlertAction.Style.default ) { [unowned self] (action: UIAlertAction) in self.dismiss(animated: true, completion: nil)
    ...
    func audioPlayerDidFinishPlaying(_ player: AVAudioPlayer, successfully flag: Bool) {
        ...
        self.updateTimeLabelText(with: 0)
        self.invalidateTime()
    }
